# FastFrame documentation

Start here:

1. [Vision & thesis](vision-and-thesis.md) — why FastFrame exists
2. [Design principles](design-principles.md) — how we decide
3. [Architecture](architecture.md) — components and stack
4. [MVP v0.1](mvp-v0.1.md) — first release
5. [Non-goals](non-goals.md) — explicit boundaries
6. [App contract](app-contract.md) — extensions and installed apps (draft)
7. [Session lifecycle](session-lifecycle.md) — database session rules (draft)
8. [CLI](cli.md) — `fastframe` vs `manage.py`
9. [Shell](shell.md) — REPL, `SHELL_IMPORTS`, startup script
10. [Public API v0.1](public-api-v0.1.md) — stable surface (draft)
11. [Roadmap](roadmap.md) — beyond v0.1
12. [Repository layout](repository-layout.md) — repo and package structure
13. [Development](development.md) — working on the framework
14. [Settings](settings.md) — admin, OpenAPI, auth, primary keys
15. [Auth](auth.md) — user model and `AUTH_USER_MODEL`
16. [Admin deployment](admin-deployment.md) — static, custom, or none
17. [Admin customization](admin-customization.md) — React admin source

Architecture Decision Records: [../adr/README.md](../adr/README.md)
